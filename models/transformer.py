...
from models.basic_class import (ACTIVATION_DICT, NORM_DICT, WEIGHT_INIT_DICT,
                            get_clones)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2, attn = self.multihead_attn(
            query=self.with_pos_embed(tgt.clone(), query_pos),  # 显式clone
            key=self.with_pos_embed(memory, pos),
            value=memory, 
            attn_mask=memory_mask,
            key_padding_mask=memory_key_padding_mask
        )
        tgt = torch.add(tgt, self.dropout2(tgt2))  # 安全加法
        tgt = self.norm2(tgt)

        # self_att
        q = k = self.with_pos_embed(tgt.clone(), query_pos)  # 显式clone
        tgt2 = self.self_attn(
            q, k, value=tgt.clone(),  # 显式clone
            attn_mask=tgt_mask,
            key_padding_mask=tgt_key_padding_mask
        )[0]
        tgt = torch.add(tgt, self.dropout1(tgt2))
        tgt = self.norm1(tgt)

        # FFN (确保激活函数非inplace)
        tgt2 = self.linear2(self.dropout(self.activation()(self.linear1(tgt))))
        tgt = torch.add(tgt, self.dropout3(tgt2))
        tgt = self.norm3(tgt)
