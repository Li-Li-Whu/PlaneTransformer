...
DATASET_ROOT_DIR = "roofpc3d/scans" #"scannet/scans"  ## Replace with path to dataset
DATASET_ROOT_DIR_ = "roofpc3d/scans_test"
DATASET_METADATA_DIR = "roofpc3d/meta_data" ## Replace with path to dataset

class Roofpc3dDatasetConfig(object):
    def __init__(self):
        self.num_semcls = 1
        self.num_angle_bin = 12
        self.max_num_obj = 12
        self.max_num_rg_center = 30
        self.K_neighbours = 10



class Roofpc3dDetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
        ...
        if split_set == "all":
            ...
            split_filenames = os.path.join(meta_data_dir, f"roofpc3d_{split_set}.txt")
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_normal:
            point_cloud = mesh_vertices[:, :]  # do not use color for now
        ...
        if self.split_set == "train":
            replace = len(point_cloud) < 4096 #2048 #4096 #8182 #16364
            choices = np.random.choice(len(point_cloud), 4096, replace=replace)
        ...
        #target_planes_normal= np.zeros((MAX_NUM_OBJ, 3), dtype=np.float32)
        ...
        '''
        for i in range(instance_planes.shape[0]):
            planes = instance_planes[i]
            angle_tgt_1[i] = planes[3]
            angle_tgt_2[i] = planes[4]
        '''
   

        #angle_tgt_1 = angle_tgt_1.astype(np.float32)
        #angle_tgt_2 = angle_tgt_2.astype(np.float32)
        ...
        #target_planes_normal[0 : instance_planes.shape[0], :] = instance_planes[:, 3:6]
